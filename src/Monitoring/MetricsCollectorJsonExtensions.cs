...
public static class MetricsCollectorJsonExtensions
{
	...
	/// <summary>
	/// Gets the shared JSON serialization options for metrics types.
	/// </summary>
	private static JsonSerializerOptions GetJsonOptions(bool indented = false) =>
		indented
	...
	public static MetricsCollector? FromJsonToMetricsCollector(string json)
	{
	...
	/// <exception cref="ArgumentException">Thrown when <paramref name="json"/> is null or empty.</exception>
	public static bool TryFromJsonToMetricsCollector(string json, out MetricsCollector? value)
	{
		value = null;
		ArgumentException.ThrowIfNullOrEmpty(json);
