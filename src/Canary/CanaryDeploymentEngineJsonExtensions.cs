...
public static class CanaryDeploymentEngineJsonExtensions
{
	...
	/// <summary>
	/// Serializes the <see cref="CanaryDeploymentEngine"/> instance to a JSON string.
	/// </summary>
	...
	public static CanaryDeploymentEngine? FromJson(string json)
	{
		...
		return JsonSerializer.Deserialize<CanaryDeploymentEngine>(json, _jsonSerializerOptions);
	}

	/// <summary>
	/// Attempts to deserialize a JSON string to a <see cref="CanaryDeploymentEngine"/> instance.
	/// </summary>
	/// <param name="json">The JSON string to deserialize.</param>
	/// <param name="value">Receives the deserialized canary deployment engine if successful.</param>
	/// <returns><see langword="true"/> if deserialization succeeded; otherwise, <see langword="false"/>.</returns>
	/// <exception cref="ArgumentNullException"><paramref name="json"/> is <see langword="null"/>.</exception>
	public static bool TryFromJson(string json, out CanaryDeploymentEngine? value)
	{
		ArgumentNullException.ThrowIfNull(json);
		...
		try
		{
			value = JsonSerializer.Deserialize<CanaryDeploymentEngine>(json, _jsonSerializerOptions);
			return true;
		}
		catch (JsonException)
		{
